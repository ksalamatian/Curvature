...
#define DEBUG_LVL 0
#define EPSILON 1*2.2204460492503131e-013
...
#include "full_transshipmentgraph.h"
...
namespace lemon {
    ...
    template <typename GR, typename V = int, typename C = V, typename NodesType = int, typename ArcsType = int64_t>
    ...
    public:
        ...
        template<typename SupplyMap>
        NetworkSimplexSimple& supplyMap(const SupplyMap* map1, int n1, int n2,const SupplyMap* map3, int n3) {
            Node n; _graph.first(n);
            for (; n != INVALIDNODE; _graph.next(n)) {
                if (n<n1)
                    _supply[_node_id(n)] = map1[n];
                ...
                if (n<n1+n2)
                    _supply[_node_id(n)] = 0.;
                else
                    _supply[_node_id(n)] = map3[n-n1-n2];
            }
            return *this;
        }
        ...
        /// function must be used, otherwise \ref resetParams() is sufficient.
        ///
        /// See \ref arcFromIdParams() for examples.
        ///
        /// \return <tt>(*this)</tt>
        ///
        /// \see resetParams(), run()
...
//            if ( !((_stype == GEQ && _sum_supply <= EPSILON ) ||
//                   (_stype == LEQ && _sum_supply >= -EPSILON )) ) {return false;}
            ...
            while (pivot.findEnteringArc()) {
                ...
                if(++niter%1000==0||niter%1000==1){
                    ...
                    for (int i=0; i<_flow.size(); i++) {
                        sumFlow0+=_state[i]*_flow[i];
                    }
                  /*  mexPrintf("Sum of the flow %.100f\n%d iterations, current cost=%.20f\nReduced cost=%.30f\nPrecision   =%.30f\n",sumFlow0,niter, curCost,_state[in_arc] * (_cost[in_arc] + _pi[_source[in_arc]] -_pi[_target[in_arc]]), -EPSILON*(a));
